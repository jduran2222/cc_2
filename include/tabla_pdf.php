...
if (isset($result_S))       // Hay subgrupos DESPLEGABLES a anidar, creamos el ARRAY de ANIDACION
 {
  ...
  foreach($tabla_subgrupo as $x => $x_value)
  {
   ...
   // creamos el content de encabezado principal
    $content_TH_ppal="<tr>";
     $c=0 ;
    if (isset($col_sel)) $content_TH_ppal.="<td><input onclick=\"selection_todo();\" type=\"checkbox\" id=\"selection_todo\" value=\"0\"></td>" ;
   ...
   $content_TH_ppal.="<th width='{$anchos_ppal[$c]}%'></th></tr>";	  
...
//   
 ...
 if ($result->num_rows > 0)
  ...
  //echo "<input type='button' id=\"btnexport\" onclick=\"tabletoXLS($idtabla)\" value=\" Export Table data into Excel \" />";  
    echo "<button type='button' class='btn btn-default btn-xs noprint'  id=\"btnexport\" >Exportar xls</button>"; 
    echo "<button type='button' class='btn btn-default btn-xs noprint'  onclick=selectElementContents(document.getElementById('$idtabla'))  >copy</button>";
   
//        BOTONOS AUMENTO DISMINUCIÓN TAMAÑO FUENTE        
   echo "<button type='button' class='btn btn-default btn-xs noprint' onclick=tabla_down_font_size(document.getElementById('$idtabla'))><i class='fas fa-search-minus'></i></button>" ;
   echo "<button type='button' class='btn btn-default btn-xs noprint' onclick=tabla_up_font_size(document.getElementById('$idtabla'))  ><i class='fas fa-search-plus'></i></button>" ;
     
   //  BOTON SELECTION
   if (isset($col_sel)) {echo "<button type='button' class='btn btn-default btn-xs noprint' onclick=table_selection()  >Selection</button>" ;  }  
...
//	   echo "<tr>";
           ...
           $c_sel=0;
           if (isset($col_sel)  ) {$content_TH_sec.= "<th><input onclick=\"selection_grupo('$idtabla');\" type=\"checkbox\" id=\"sel2_$idtabla\" value=\"0\"></th>" ;  $c++ ;$c_sel=1; }       // Si hay columna de Selección añadimos una columna vacía a los TOTALES
...
//    echo $content_TH_sec;        // pintamos el encabezado principal
...
//	         echo "<td align='right'> <button type='button' class='btn btn-link btn-xs' data-toggle='collapse' data-target='#div_$id_subgrupo'>expand</button></td></tr>";	  
	         echo "<td align='right'> <button type='button' class='btn btn-link btn-xs' data-toggle='collapse' data-target='#div_$id_subgrupo'>expand</button></td></tr>";	  


//               echo "<tr ><td COLSPAN=3></td></tr>";
//   debog 1     echo "</table><div id='div_$id_subgrupo' class='collapse'><table>";
//                 echo "<tr><td colspan='40'><div id='div_$id_subgrupo' class='collapse'><table id=\"$idtabla\" class='table2' >";
...
//          if (isset($col_sel)  ) {echo  "<td><input type=\"checkbox\" id=\"{$rs[$col_sel]}\" value=\"{$rs[$col_sel]}\"></td>" ;  }       // Si hay columna de Selección añadimos una columna vacía a los TOTALES
          if (isset($col_sel)  ) {echo  "<td><input type=\"checkbox\" id=\"sel_$idtabla\" value=\"{$rs[$col_sel]}\"></td>" ;  }       // Si hay columna de Selección añadimos una columna vacía a los TOTALES
           ...
           if (isset($actions_row["id"]))                          
                     {
                      ...
                      echo "<td>" ;
                       ...
                       if (isset($actions_row["delete_link"])) 
                           ...
                           echo "<a class='dentro_tabla' href=# onclick=\"tabla_delete_row('$cadena_link')\">borrar</a> "; 
                           }
...
//	  }  
 ...
 { echo  "<p>$msg_tabla_vacia</p>" ;}
  ...
  // INICIALIZAMOS TODAS LAS VARIABLES Y ARRAYS DE TABLA.PHP PARA EVITAR INTERFERENCIAS CON NUEVAS REQUERY EN EL MISMO PHP.
      ...
      //  $result=$Conn->query("SELECT id_aval , Motivo, Importe from Avales WHERE ID_OBRA=".$id_obra );
        unset( $titulo, $msg_tabla_vacia,  $add_link,  $links, $format, $format_style, $tooltips, $actions_row, $updates,$dblclicks ,$id_agrupamiento ) ;
