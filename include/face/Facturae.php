...
class Facturae {
   ...
   */
  public function addItem($desc, $unitPrice=NULL, $quantity=1, $taxType=NULL, $taxRate=NULL, $exp='') {
    if ($desc instanceOf FacturaeItem) {
      ...
      $item = new FacturaeItem([
        ...
        "exp" => $exp                                      // añadido JUAND , mayo-2018    
      ]);
    }
  ...
  public function export($filePath=NULL) {
    ...
    foreach ($this->items as $itemObj) {
      $item = $itemObj->getData();
      $xml .= '<InvoiceLine>' .
                '<FileReference>' . $item['exp'] . '</FileReference>' .
