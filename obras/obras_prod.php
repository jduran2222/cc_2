...
if (!$rs_obra = Drow("OBRAS", " $where_c_coste AND ID_OBRA={$_GET["id_obra"]}"))
{
    cc_die("ERROR, datos inconsistentes");
}      // comprobacion SEGURIDAD


$titulo_pagina = "RVS " . $rs_obra["NOMBRE_OBRA"];
            ...
            if (!(nuevo_valor === null || nuevo_valor === ""))
            {
                ...
                xhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        }  // refresco la pantalla con el n uevo caoítulo creado
                    }
                };
...
// comprobamos si id_prod_estudio_coste está actualizado y en caso negativo lo actualizamos (hacemos un clon del Proyecto en la RV Estudio de Coste)
        if (!cc_is_ESTUDIO_COSTE_actualizado($id_obra))
        {
...
//  $rs["IMPORTE"]= $rs["IMPORTE"] ? $rs["IMPORTE"] : 1 ;  // si rs importe es CERO lo ponemos a 1 para evitar el error
            ...
            $borrar_disable = ($id_produccion <> $rs_obra["id_prod_estudio_costes"] AND $id_produccion <> $rs_obra["id_produccion_obra"] ) ? "" : "disabled";
...
//    $porc_iva_incluido=number_format($rs["Val_iva_incluido"]*100/$rs["IMPORTE"],2)."%";
            ...
            <tr>
                ...
                <td align=right>
                    ...
                    <button class="btn btn-danger btn-xs"   <?php echo $borrar_disable; ?> title='Borra la Relación Valorada, excepto la Rel. Val. PROYECTO y la Rel. Val.PRODUCCION OBRA'
